...
import { Bell, LogOut, Loader2, Settings, HelpCircle, ArrowRight } from "lucide-react";
...
export function HeaderActions() {
  ...
  // 3. Secure logout
  const handleLogout = () => {
    document.cookie = "tracker_token=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;";
    queryClient.clear();
    router.push("/login");
  };
  ...
  const calcPos = (ref: React.RefObject<HTMLButtonElement>) => {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      >
        {isLoading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : userInitial}
      </button>
        ...
        createPortal(
          ...
          >
            ...
            <div className="px-4 py-3.5 border-b border-[#e0e0e0]/60 flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-[#1f1f1f]">Notifications</p>
                <p className="text-[11px] text-[#9aa0a6] mt-0.5">Recent pipeline activity</p>
              </div>
              {unreadCount > 0 && (
            ...
            <div className="max-h-80 overflow-y-auto">
              {notifications.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-10 text-center px-4">
                  ...
                  <p className="text-sm font-medium text-[#1f1f1f]">All caught up</p>
                  <p className="text-xs text-[#9aa0a6] mt-0.5">No new leads in the last 7 days.</p>
                </div>
              ) : (
                <div className="p-1.5 space-y-0.5">
                  {notifications.map((n) => {
                    const dotColor = STATUS_DOT[leads.find((l) => l.id === n.leadId)?.status ?? ""] ?? "bg-[#888780]";
                    const bgColor  = STATUS_COLOR[leads.find((l) => l.id === n.leadId)?.status ?? ""] ?? "bg-[#f1efe8]";
                    return (
                      <button
                        key={n.id}
                        onClick={() => { router.push(`/leads/${n.leadId}`); closeNotif(); }}
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between gap-2">
                            <p className="text-sm font-medium text-[#1f1f1f] truncate">{n.title}</p>
                            <ArrowRight className="w-3.5 h-3.5 text-[#d1d5db] group-hover:text-[#3186ff] shrink-0 transition-colors" />
                          </div>
                          <p className="text-xs text-[#9aa0a6] mt-0.5 truncate">{n.subtitle}</p>
                          <p className="text-[11px] text-[#c4c7c5] mt-0.5">{timeAgo(n.timestamp)}</p>
                        </div>
                      </button>
                    );
                  })}
            ...
            {notifications.length > 0 && (
              <div className="p-1.5 border-t border-[#e0e0e0]/60">
                <button
                  onClick={() => { router.push("/leads"); closeNotif(); }}
      ...
      {profileOpen &&
        createPortal(
          ...
          >
            ...
            <div className="px-4 pt-4 pb-3 border-b border-[#e0e0e0]/60">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-sm font-medium text-[#1f1f1f] truncate">{user?.email}</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-1.5">
              <button
                onClick={() => { router.push("/settings"); closeProfile(); }}
              ...
              <button
                onClick={() => { router.push("/help"); closeProfile(); }}
                className="w-full flex items-center gap-2.5 px-3 py-2 text-sm font-medium text-[#444746] hover:bg-[#f8f9fa] hover:text-[#1f1f1f] rounded-xl transition-colors"
              >
            ...
            <div className="p-1.5 border-t border-[#e0e0e0]/60">
              <button
                onClick={() => { handleLogout(); closeProfile(); }}
